...
function validate_checkin_withres(output){
	    t1 = valid_length('username1');
		var x1 = document.forms["checkinformwithres"]["roomnumber"].value;
		var numbers = /^[0-9]{1,3}$/;
	
  errorlist = '';
    if (! t1) {
        errorlist += 'Missing Username<br />';
		
    }
	if(!x1.match(numbers)){
		errorlist += 'Invalid Roomnumber<br />';
	}
	if (errorlist) {
        document.getElementById(output).innerHTML = errorlist;   
		return false;
...
function validate_bill(output){
	...
	errorlist = '';
	if (! t1) {
        errorlist += 'Missing Room Number<br /><br/>';
    }
	if (! t2) {
