...
$error=0;
	...
	if(isset($_POST['start'])){
		...
		and (strlen($room) >= 1 and strlen($room) <= 3)){
			...
			$que= "select * from room where room=".$room;
			$result = pg_query($DBConnection->conn, $que);
			...
			if(pg_num_rows($result)==0)
			{
					...
					$query5= "INSERT into bill(roomnumber, amount, serviceavailed, dateavailed) values ($room, 5000, 'Room Charge', current_date)";
