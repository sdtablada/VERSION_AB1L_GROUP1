...
<!-- Background slideshow-->
	<div class="slideshow">
		<img src="images/bg5.jpg" id="bg1"/>
		<img src="images/bg2.jpg" id="bg2"/>
		<img src="images/bg3.jpg" id="bg3"/>
		<img src="images/bg4.jpg" id="bg4"/>
		<img src="images/bg1.jpg" id="bg5"/>
		<img src="images/bg6.jpg" id="bg6"/>
		<img src="images/bg7.jpg" id="bg7"/>
	</div>
<!-- Short description -->
	<div id="aboutthehotel">
		The most recognized name in the industry, Atalia Hotels & Resorts 
		stands as the stylish, forward thinking global leader of hospitality.
		We help make traveling easier with our smart design and innovative 
		restaurant concepts.
		<p>We invite you to experience everything we have to offer, 
		from our contemporary settings to our nourishing and delicious 
		dining options and more. Featuring premium amenities and a 
		tranquil setting, each of our guest rooms worldwide is 
		uniquely designed to be both smart and completely accommodating. </p>
		<p>Feel free to contact us at 289-76-61.</p>
	</div>
...
<!-- SIGN IN -->	
	<?php 
		...
		{ 
			echo "<div id=\"signinform\" style=\"display: block \">";
		}
		else
		{
	...
	?>
		...
		<form method="post" name="logForm" onsubmit="return valsignin('signresult');" action="login.php">
			<br/>
				<div class="bodytext">Username &nbsp;<input type="text" name="username" id="username" />
...
<!-- REGISTER -->
	<?php 
		...
		{ 
			echo "<div id=\"regform\" style=\"display: block \">";
		}
		else
		{
			echo "<div id=\"regform\" style=\"display: none \">";
		}
	?>
		...
		<table id="regtable">
			<form method="post" name="regForm" onsubmit="return validate_all('results');" action="register.php">
				<tr>
					...
					<td rowspan="6"><div id="results"><?php if(isset($_SESSION['regerr'])){ echo "Username already already taken";
					unset($_SESSION['regerr']);} ?></div></td>
				</tr>
				<tr>
...
<!-- ABOUT THE HOTEL -->
		<div id="aboutatalia">
			...
			and the smart design, thoughtful amenities and innovative dining options that make us a global leader in hospitality.
			The most recognized name in the industry, Atalia Hotels & Resorts stands as the stylish, forward thinking global leader of 
			hospitality. With over 92 years of experience, Atalia continues to be synonymous with hotel because of our innovative approach 
			to products, amenities and service.  <br/>
			</div>
				...
				<table id="reservetable">
					<form name="resForm" id="resForm" onsubmit="return validate_reservation('resresult');" action="reserveroom.php" method="post">
						...
						<tr>
							...
							<?php 
								while($rooms = pg_fetch_row($resquery))
									{
										echo "<option value = '$rooms[0]'>$rooms[0]</option>";
									}
							?>
							</select></td>							
							<td><input type="submit" value="Submit Reservation" id="reserve" name="reserve"/></td>
						</tr>
							<?php 
								if(isset($_SESSION['reserved'])) 
									echo "<script>alert(\"Room {$_SESSION['reserved']} has been reserved!\");</script>"; 
		...
		<div id="listofservices">
               <?php
                    ...
                    if($count>0)
					{
						...
						echo "<div class='bodytext1'>No services available</div>";
                    echo "</table>";
					if(isset($_SESSION['availed'])) 
						echo "<script>alert(\"{$_SESSION['availed']} availed!\");</script>"; 
						unset($_SESSION['availed']);
					if(isset($_SESSION['not'])) 
						echo "<script>alert(\"You must check-in first before availing services.\");</script>"; 
						unset($_SESSION['not']);
				?>
		</div>
...
<!-- BILLING RECORDS-->
		<div id="billingrecords">
			<table id="billtable">
				...
				{
					echo "<div class=\"bodytext1\">User not checked-in or has no reservation</div>";
				}				
		?>
			</table>
			<br/>
...
<!-- ROOM STATUS -->
	<?php 
		if(isset($_SESSION['all'])||isset($_SESSION['viewvacant'])||isset($_SESSION['viewreserved']))
		{ 
			echo "<div id=\"rmstatForm\" style=\"display: block \">";
		}
		else
		{
			echo "<div id=\"rmstatForm\" style=\"display: none \">";}
	?> 
		<form name="checkroomstat" id="checkroomstat" action="viewroomstat.php" method="post">
			...
			<input type="submit" name="viewvacant" id="vacant" value="Reserved Rooms"/>
				...
				if(isset($_SESSION['all']))
				{
						...
						$q1 = pg_query($DBConnection->conn, "SELECT * FROM \"room\" ORDER BY roomnumber");
							...
							unset($_SESSION['all']);
								...
								$q2 = pg_query($DBConnection->conn, "SELECT * FROM \"room\" WHERE roomstatus = 'reserved' ORDER BY roomnumber");
						...
						<th class="cell">Departure Date</th>
								...
								$q3 = pg_query($DBConnection->conn, "SELECT * FROM \"room\" WHERE roomstatus = 'occupied' ORDER BY roomnumber");
...
<!-- ADD/DELETE SERVICE -->
	<?php 
		if(isset($_SESSION['serviceadded'])||isset($_SESSION['servicenotadded'])||isset($_SESSION['servicedeleted'])||isset($_SESSION['noservice']))
		{ 
			echo "<div id=\"serviceForm\" style=\"display: block \">";
		}
		else
		{
		echo "<div id=\"serviceForm\" style=\"display: none \">";
		}
	?> 	 	
	<?php
	   $show = pg_query($DBConnection->conn, "SELECT * FROM services where servicename not like 'Room Charge'");
	   $count = pg_num_rows($show);
			...
			echo "<thead><tr class=\"cell\"><th> Service name </th><th  align=\"center\"> Amount </th></tr></thead>";
				for($i=0;$i<$count;$i++)
				{ // show all services currently in the database
						echo "<tr class=\"cell\">";
						$row = pg_fetch_array($show);
...
<!-- BILL CUSTOMER -->
	<?php 
		...
		{
			echo "<div id=\"adminbillingform\" style=\"display: block \">";
		}
		else
		{
		echo "<div id=\"adminbillingform\" style=\"display: none \">";
		}
	?>
		...
		<form action="billcustomer.php" method="post" onsubmit="return validate_bill('div_bill');" name="billcustomer" id="billcustomer">
			<?php
					if (isset($_GET['error']))
					{	
						$error= $_GET['error'];if($error==1) 
						echo '<center><div class="bodytext">Room not occupied.</div></center>';
					}
			?>
		<table id="adminbilling">
			...
			<tr>
				...
				<td>
					<?php
							$services = pg_query($DBConnection->conn, "SELECT * FROM services where servicename not like 'Room Charge'");
...
<!-- CHECK-IN -->
	...
	<?php 
		if(isset($_SESSION['res'])||isset($_SESSION['nores'])||isset($_SESSION['checkin2']))
		{
			echo "<div id=\"checkinForm\" style=\"display: block \">";
		}
		else
		{
		echo "<div id=\"checkinForm\" style=\"display: none \">";
		}
	?>
			...
			<input type="submit" name="walk" value="Check in without reservation"/><br/>
				<?php 
					if(isset($_GET['error']))
					{
						$error= $_GET['error'];
							if ($error==1) 
								echo '<div class=\"bodytext1\">The customer has not reserved the specified room number</div>';	
							else if($error==2) echo '<div class=\"bodytext1\">The user is currently checked in';
					}
				?>
		</form>
<!--end of form-->
<!--checkin with reservation-->
				...
				<form method="post" name="checkinform" id= "checkinOpt" onsubmit="return validate_walkin('walkin_res1');"  action="checkin.php">	
					<center>
						...
						<select id="room" name='room'>
							<?php 
								$resquery ="SELECT * FROM ROOM WHERE ROOMSTATUS='vacant' order by roomnumber";
								$result= pg_query($DBConnection->conn, $resquery);
								while($rooms = pg_fetch_row($result))
									{
										echo "<option value = '$rooms[0]'>$rooms[0]</option>";
									}
							?>
						...
						<br/>
							<input type="text" name="arrdate" id="arrdate" placeholder="Arrival Date(YY-DD-MM)" maxlength="10" /> <br />
							<input type="text" name="depdate" id="depdate" placeholder="Departure Date(YY-DD-MM)" maxlength="10" /> <br />
							<input type="submit" name="start" value="Check In" />
							<input type="submit" name="cancel" value="Cancel"/><br />	
							<div id="walkin_res1"></div>	
						</center>					
				</form>	
